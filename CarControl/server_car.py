...
def calibrate(pi,ESC):   # Стандартная процедура автокалибровки для esc регулятора
    ...
    if inp == '':
        ...
        print("Подключите батарею прямо сейчас. Вы должны услышать 2 звуквых сигнала. Затем дождитесь окончания сигнала"
              " и нажмите Enter")
        inp = input()
        if inp == '':
...
def main():
    # Перед началом работы программа выведет строку 'waiting for a connection', после этого можно отправлять команды
